...
src_install() {
	...
	newinitd ${FILESDIR}/kerio-connect.init kerio-connect
	newconfd ${FILESDIR}/kerio-connect.conf kerio-connect

	if use systemd; then
		systemd_newunit "${FILESDIR}"/kerio-connect.systemd kerio-connect.systemd
	fi
